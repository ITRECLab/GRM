...
# parser.add_argument("--weight_label", action='store_true')#for test ner,add!
...
def train_RC(model, iterator, dataset, epoch):
    ...
    for i, batch in enumerate(iterator):
        ...
        model.optimizer.zero_grad()
        logits, loss_sc, y, y_hat = model.forwardRC(x, labels, P_gauss1_list, P_gauss2_list)#, P_gauss1_list, P_gauss2_list

        loss1 = criterion(logits, y)
        # loss_sc = loss_sc.cuda()
        loss = 0.6*loss1 + 0.4*loss_sc #/ (loss_sc/loss).detach()
        loss.backward()
...
def eval_RC(model, iterator, epoch, documents_eval, documents_eval_ground_truth):
    ...
    with torch.no_grad():
        for i, batch in enumerate(iterator):
            x, labels, guid, P_gauss1_list, P_gauss2_list = batch#, P_gauss1_list, P_gauss2_list
            logits, loss_sc, y, y_hat = model.forwardRC(x, labels, P_gauss1_list, P_gauss2_list)#, P_gauss1_list, P_gauss2_list
...
def train_Joint(model, iterator_a, iterator_b, task_a, task_b, epoch):
    ...
    def train_RC_batch(model, batch):
        ...
        x, labels, guid, P_gauss1_list, P_gauss2_list = batch
        model.optimizer.zero_grad()
        logits, loss_sc, y, y_hat = model.forwardRC(x, labels, P_gauss1_list, P_gauss2_list)
        ...
        if args.do_train:
            ...
            if args.task.lower() == 'bc2gm':
                ...
                if args.do_cross_valid:
                    if fold_iter_idx == 0:
                        if args.train_withGNP:
                            train_data_dir = os.path.join('D:/code/BC6PM', 'GNormPlus', 'withAnn-Result', 'PMtask_Relations_TrainingSet_r.json')#os.environ.get('BC6PM_dir')
                        else:
                            train_data_dir = os.path.join('D:/code/BC6PM', 'json', 'PMtask_Relations_TrainingSet.json')
                    ...
                    # Triage Data
                    if fold_iter_idx == 0:
                        with open(os.path.join('D:/code/BC6PM', 'json', 'PMtask_Triage_TrainingSet.json')) as f:
                    ...
                    if args.train_withGNP:
                        train_data_dir = os.path.join('D:/code/BC6PM', 'GNormPlus', 'withAnn-Result', 'PMtask_Relations_TrainingSet_r.json')
                    else:
                        train_data_dir = os.path.join('D:/code/BC6PM', 'json', 'PMtask_Relations_TrainingSet.json')
                    with open(train_data_dir) as f:
                        train_data = json.load(f)
                    ...
                    with open(os.path.join('D:/code/BC6PM', 'GNormPlus', 'result', 'PMtask_Relations_TestSet_r.json')) as f:
                        eval_data = json.load(f)
                    documents_eval = eval_data['documents']
                    with open(os.path.join('D:/code/BC6PM', 'json', 'PMtask_Relations_TestSet.json')) as f:
                    ...
                    with open(os.path.join('D:/code/BC6PM', 'json', 'PMtask_Triage_TestSet.json')) as f:
